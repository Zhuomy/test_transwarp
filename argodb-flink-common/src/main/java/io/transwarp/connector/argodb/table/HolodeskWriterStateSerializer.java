...
class HolodeskWriterStateSerializer implements SimpleVersionedSerializer<HolodeskWriterState> {
  private static final Logger LOG = LoggerFactory.getLogger(HolodeskWriterStateSerializer.class);
  ...
  @Override
  public byte[] serialize(HolodeskWriterState state) throws IOException {
...
//            out.write(transactionPB);
  ...
  @Override
  public HolodeskWriterState deserialize(int version, byte[] serialized) throws IOException {
    try (final ByteArrayInputStream bais = new ByteArrayInputStream(serialized);
      ...
      return new HolodeskWriterState(transactionalId, ShivaEnv.getShiva2Client(masterGroup), shivaBulkTransaction);
    } catch (ShivaException | io.transwarp.shiva2.shiva.exception.ShivaException e) {
